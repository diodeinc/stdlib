...
def _unit_to_str_with_tolerance(self):
    """Convert a unit record to a string representation with SI prefix and tolerance."""
    # Extract the value and tolerance
    value = self.value
    tolerance = self.tolerance * 100  # Convert to percentage
...
Capacitance = record(
    ...
    tolerance=field(float, 0.1),
    __str__=field(typing.Callable, _unit_to_str_with_tolerance),
)
Resistance = record(
    value=float,
...
def _check_prefix(unit: str, unit_type: str) -> float:
    ...
    prefix = normalized_unit.lower().replace(unit_type.lower(), "")
